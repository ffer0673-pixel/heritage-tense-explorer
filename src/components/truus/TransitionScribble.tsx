...
import { ANIMATION_CONFIG } from '@/lib/truus-data';

export default function TransitionScribble() {
    useEffect(() => {
        const logoTruusClickable = document.querySelector('.logo-truus') as HTMLElement | null;
        const transitionScribblePath = document.querySelector('.transition-scribble path') as SVGPathElement | null;
        const transitionScribbleSvg = document.querySelector('.transition-scribble') as HTMLElement | null;
        ...
        const runScribbleAnimation = (e: Event | null) => {
            ...
            if (!transitionLogo) {
                ...
                const logoElement = document.querySelector('.logo-truus');
                const svgClone = logoElement ? logoElement.cloneNode(true) as HTMLElement : null;
                if (svgClone) {
            ...
            drawTl.to(transitionLogo, {
                ...
                onStart: () => {
                    ...
                    if (logoSvg) gsap.to(logoSvg, { rotation: 5, duration: 0.15, repeat: -1, yoyo: true, ease: 'steps(1)', overwrite: 'auto' });
                }
            }, durIn * 0.5);
