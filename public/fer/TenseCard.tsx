...
function TenseCardImpl({ tense, index, total, scrollYProgress, onLearnMore }: TenseCardProps) {
  ...
  // Blur is by far the most expensive property here (forces a repaint,
  // not just compositing), so it's kept modest and only ever applied to
  // the handful of cards the parent keeps mounted around the active one.
  const blurPx = useTransform(p, [-1, 0, 1], [6, 0, 5]);
  const filter = useTransform(blurPx, (v) => (v < 0.3 ? 'none' : `blur(${v}px)`));
...
// Memoized: props (tense/index/total/onLearnMore) are stable across parent
// re-renders — only the shared scrollYProgress motion value "changes", and
// motion values don't trigger re-renders on their own. Without memo, every
// windowing re-render of the parent would still re-render every mounted card.
export const TenseCard = memo(TenseCardImpl);

const TenseCardBody = memo(function TenseCardBody({
  tense,
  imageY,
  p,
  onLearnMore,
